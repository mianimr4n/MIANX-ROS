-- Canonical menu catalog sync (forward-only content migration).
-- Generated/reviewed from: data/catalog/telepizza-canonical-menu.json
--
-- HARD RULES:
--   * Do NOT apply to production without owner approval.
--   * Upsert by slug/code only — never treat UUID as identity.
--   * Deactivate obsolete rows (is_available / is_active = false).
--   * NO destructive DELETE of catalog rows (preserve order history FKs).
--   * Database V1 Freeze PASS LOCKED for schema — this is content-only.
--
-- Dry-run checklist (run SELECT blocks first; do not COMMIT on prod until approved):
--   1) Counts before / after must remain browse freeze-compatible OR owner-approved bump.
--   2) Discontinued broast + behari-kabab-pizza stay unavailable.
--   3) Modifier option price_delta for linked SKUs align to menu_items.base_price.
--   4) No new sellable SKUs from ownerGapsNotInV1Sellable without explicit approval.
--
-- Schema readiness: existing menu_* + modifier_* + item_modifier_groups can represent
-- sauces (as Dips SKUs), drinks, toppings, crust, add-ons. No schema change required.
-- STOP condition: none for representation; BLOCKER is owner menu-board image evidence
-- for GM-gap domains / unverified modifier seed prices (see canonical completionStatus).

begin;

-- ---------------------------------------------------------------------------
-- 1) Categories — upsert sellable + keep toppings hidden + broast discontinued
-- ---------------------------------------------------------------------------
...
values
  ...
  ('Toppings', 'toppings', 200, false),
  ('Broast', 'broast', 999, false)
...
-- ---------------------------------------------------------------------------
-- 2) Ensure discontinued browse SKUs stay unavailable
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- 3) Upsert V1 hybrid sellable + topping SKUs (codes from canonical manifest)
--     Prices = board-era BFR-001 hybrid (matches current prod). Idempotent.
-- ---------------------------------------------------------------------------
...
values
  ...
  ((select id from public.menu_categories where slug = 'toppings'), 'extra-chicken', 'Extra Chicken', 'Extra chicken topping for pizza. Size tier matches small / medium / large pizza.', '/images/menu-pizza.jpg', null, null, 'topping', true, false),
  ((select id from public.menu_categories where slug = 'toppings'), 'extra-cheese', 'Extra Cheese', 'Extra cheese topping for pizza. Size tier matches small / medium / large pizza.', '/images/menu-pizza.jpg', null, null, 'topping', true, false),
  ((select id from public.menu_categories where slug = 'toppings'), 'extra-cheese-slice', 'Extra Cheese Slice', 'Extra cheese slice topping for pizza (single verified price).', '/images/menu-pizza.jpg', 60, null, 'topping', true, false)
on conflict (slug) do update
set
...
-- ---------------------------------------------------------------------------
-- 4) Size-matrix variants (board-era)
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- 5) Align linked modifier option price_delta to catalog SKU prices
--     (add-drinks / add-sides / extra-cheese-slice). Does not invent new groups.
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- Verification (dry-run / post-apply):
-- select count(*) from menu_categories where is_active and slug not in ('toppings'); -- 13
-- select count(*) from menu_items where is_available and product_type <> 'topping'; -- 58
-- select count(*) from menu_items where product_type = 'topping' and is_available; -- 3
-- select count(*) from menu_item_variants v join menu_items i on i.id = v.menu_item_id
--   where v.is_available and i.is_available; -- 40
-- select slug from menu_items where is_available and slug like '%broast%'; -- 0
--
-- Rollback guidance: re-run prior owner sync 20260716160000; no DELETE required.
-- OWNER APPROVAL REQUIRED before production apply.
-- ---------------------------------------------------------------------------
