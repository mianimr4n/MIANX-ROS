...
> **Superseding freeze verdict (2026-07-18 owner scope):**
> See `_documentation-audit/reports/DATABASE-CORE-RESTAURANT-PRE-FREEZE-ARCHITECTURE.md`.
> P0/P1 findings below remain valid **and** core restaurant foundations are now mandatory.
...
Migration history is **aligned** and dry-run is **clean** (same posture as PR #61 unblocked history work). Freeze is still **BLOCKED** by privilege drift and unmanaged `public.profiles` — and, under the owner scope change, by missing restaurant foundation schemas.
...
History/dry-run were green at audit time; apply P0 (+ P1 profiles disposition) **and** core restaurant foundation slices (DB-R2…R7) before declaring the database frozen. See `DATABASE-CORE-RESTAURANT-PRE-FREEZE-ARCHITECTURE.md`.
