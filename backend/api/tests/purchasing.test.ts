...
const purchasing: PurchasingService = {
  ...
  async createInvoice(_scope, input) {
    return {
      ...
      isOverdue: false,
      settlementBlocked: Boolean(input.purchaseOrderId),
      createdAt: new Date().toISOString(),
    };
  },
  async approveInvoiceException(_scope, _actorUserId, invoiceId, input) {
    return {
      id: invoiceId,
      branchId: BRANCH_ID,
      branchCode: "royal-orchard",
      ...
      matchingStatus: "DISCREPANCY" as const,
      exceptionApprovedAt: new Date().toISOString(),
      exceptionApprovedBy: "user-admin",
