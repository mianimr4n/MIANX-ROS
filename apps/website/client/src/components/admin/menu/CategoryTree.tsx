  ...
  return (
    ...
    >
      <AdminSectionTitle
        eyebrow="Structure"
        title="Category tree"
        description="Flat category list from catalog — nested hierarchy Planned for Phase 2 until parent/child relations exist in API."
      />
      {loading ? (
      ...
      <p className="mt-4 rounded-lg border border-dashed border-[var(--admin-border)] px-3 py-2 text-xs text-[var(--admin-muted)]">
        Multi-level trees (Pizza → Classic → Premium) require category parent relations — Planned for Phase 2.
      </p>
    </section>
  );
}
