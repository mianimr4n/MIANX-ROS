...
import type { HrIntegrationCheck, HrReadinessGroup } from "@/lib/admin-hr";
import { AdminSurface, AdminSurfaceBody, AdminSurfaceHeader } from "@/components/admin/AdminSurface";

export function HRFoundationPanel({ checks }: { checks: HrIntegrationCheck[] }) {
  return (
    <AdminSurface aria-labelledby="hr-integration-heading" className="mb-6">
      <AdminSurfaceHeader title="Integration readiness" description="Verified repository workforce dependencies." />
      <AdminSurfaceBody>
        <h2 id="hr-integration-heading" className="sr-only">
        ...
        <p className="mt-4 text-xs text-[var(--admin-muted)]">
          ...
          {" · "}
          <Link href="/admin/reports" className="font-semibold text-[var(--brand-red)] underline">
            Reports
          </Link>
        </p>
      </AdminSurfaceBody>
...
export function HRReadinessSections({ groups }: { groups: HrReadinessGroup[] }) {
  return (
    <section aria-labelledby="hr-readiness-sections" className="mb-6 grid gap-4 lg:grid-cols-2">
      <h2 id="hr-readiness-sections" className="sr-only">
        HR foundation requirements
      </h2>
      {groups.map((group) => (
