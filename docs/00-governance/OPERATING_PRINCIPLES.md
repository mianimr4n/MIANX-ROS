...
## Principles
1. Repository evidence beats planning documents.
2. Owner/Founder are display labels; authorization is `super-admin` + global scope.
3. Never invent LIVE zeros from failed API calls.
4. Northern Bypass stays coming-soon until Founder authorization.
5. Do not mutate Production business data during readiness documentation work.
6. Decision Log is an Append-only Record.
7. One active opening-readiness branch: `feature/opening-readiness-final`.
