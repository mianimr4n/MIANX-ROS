...
**Last reconciled:** 2026-08-16 — **Phase 12 COMPLETE (`v2.7.0`)** — Customer and Staff Apps formally closed: ADR-039/040/041 accepted (Customer Mobile & Franchise Portal Contract, Rider Mobile App & Delivery Dashboard Contract, Staff App & Support Panel Contract). Phase 12 is closeout-only — no new migrations applied (Production DB tip remains `20260821000000`, same as Phase 5/6/7/8/9/10/11). All customer/staff/rider/franchise/support/delivery surface (`apps/website` React + Vite SPA + 25+ customer pages + PWA manifest + 37 admin pages + 5 ops pages + 32 admin router modules totaling 350+ routes + 4 rider-facing routes + 10 admin delivery routes + 8 delivery dashboard sub-components + 8 CRM routes + 11 WhatsApp routes + `organization_owner` role + `getOwnerWorkspace` 25-module analytics + `AdminBranchManager.tsx` multi-branch view + ADR-019 RBAC with `branch_manager`/`kitchen_manager`/`cashier`/`rider`/`support` roles + ADR-027/028/029 KDS + ADR-023/024/025/026 POS + ADR-012 audit log) was already verified during prior phases' migration runs; `scripts/phase_12_verify.py` provides customer/staff-apps-focused re-verification (70+ checks across 10 categories) as a future artifact. Phase 11 (v2.6.0) previously closed: ADR-036/037/038 accepted, closeout-only. Phase 10 (v2.5.0) previously closed: ADR-033/034/035 accepted. Phase 9 (v2.4.0) previously closed: ADR-030/031/032 accepted. Phase 8 (v2.3.0) previously closed: ADR-027/028/029 accepted. Phase 7 (v2.2.0) previously closed: ADR-023/024/025/026 accepted. Phase 6 (v2.1.0) previously closed: ADR-019/020/021/022 accepted. Phase 5 (v2.0.0) previously closed: ADR-018 accepted. Phase 3 (v1.10.0 ADR-016/017 OTP) merged earlier as PR #231 — squash `2967a1c`. Repository main now `4c97b6c` (Phase 11 closeout — PR #238) → Phase 12 closeout pending PR merge; Production migration tip `20260821000000` (no new migrations in Phase 12 — closeout only); backend tests unchanged from v2.6.0 (closeout-only release); prior `v2.6.0` @ `4c97b6c…` unchanged
...
| --- | --- | --- |
| Repository main (current tip) | `4c97b6c` (Phase 11 closeout — PR #238) | ADR-036..038 (Finance and Reporting); Phase 12 closeout (ADR-039..041) pending v2.7.0 tag |
| Latest released baseline | `v2.7.0` @ (pending tag — Phase 12 closeout) | **Phase 12 COMPLETE** — ADR-039/040/041 accepted; Customer and Staff Apps surface Production-verified (closeout-only, no new migrations; reuses Phase 5/6/7/8/9/10/11 baseline `20260821000000`) |
| Prior released baseline | `v2.6.0` @ `4c97b6c...` | **Phase 11 COMPLETE** — ADR-036/037/038 accepted; Finance and Reporting surface Production-verified (closeout-only, no new migrations; reuses Phase 5/6/7/8/9/10 baseline `20260821000000`) |
...
| Phase 9 Rider and Delivery App (ADR-030/031/032) | **CLOSED** — `v2.4.0` tagged | Rider Identity/Dispatch/Assignment Contract + Delivery Lifecycle/Pickup/POD Surface + Rider Location/Navigation/Performance Contract formally accepted as ADRs; Production-verified (closeout-only — no new migrations; reuses Phase 5/6/7/8 baseline `20260821000000`) |
| Phase 10 Inventory and Procurement (ADR-033/034/035) | **CLOSED** — `v2.5.0` tagged | Inventory Stock Master/Movement Ledger + Recipe/BOM/COGS Costing + Procurement/Suppliers/GRN formally accepted as ADRs; Production-verified (closeout-only — no new migrations; reuses Phase 5/6/7/8/9 baseline `20260821000000`) |
| Phase 11 Finance and Reporting (ADR-036/037/038) | **CLOSED** — `v2.6.0` tagged | Branch GL/P&L/Balance Sheet/Cash Flow + Cash Reconciliation/Z-Report/COD Financial Ownership + Tax/AR/AP/COGS/Expense Posting formally accepted as ADRs; Production-verified (closeout-only — no new migrations; reuses Phase 5/6/7/8/9/10 baseline `20260821000000`) |
| Phase 12 Customer and Staff Apps (ADR-039/040/041) | **CLOSED** — pending `v2.7.0` tag | Customer Mobile & Franchise Portal + Rider Mobile App & Delivery Dashboard + Staff App & Support Panel formally accepted as ADRs; Production-verified (closeout-only — no new migrations; reuses Phase 5/6/7/8/9/10/11 baseline `20260821000000`) |
...
| Architecture | **Approved (Phase 12 COMPLETE — ADR-001 through ADR-041 all Accepted v1.0)** |
| Requirements | Active |
