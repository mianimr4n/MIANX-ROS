...
| Confirmation required | No session until email is confirmed; UI shows a generic success + spam reminder |
| Resend | Rate-limited “Resend confirmation email” via `supabase.auth.resend({ type: "signup" })` |
...
2. Configure a production provider (Resend, Postmark, SendGrid, Amazon SES, etc.)
3. Use a verified sender domain (see §5)
4. Save and send a test confirmation from a fresh test mailbox
...
| Requirement | Why |
|---|---|
| SPF | Authorizes the SMTP provider to send for your domain |
| DKIM | Cryptographic signature; reduces spam scoring |
| DMARC | Policy alignment for the From domain |
| Matching From domain | Prefer `noreply@yourdomain` over a shared Supabase default |

Until DNS is verified, confirmation mail may land in spam or be dropped.
...
4. Website enforces a client cooldown on Resend; do not remove it to “fix” delivery
...
2. Confirm UI shows the generic success message (no auto-login when confirmation is required).
3. Wait for the Resend cooldown, then click **Resend confirmation email**.
4. Open the link → lands on `/auth/callback` → session established → safe internal redirect.
5. Confirm Auth Logs show the resend attempt.

---

## 8. Confirmation callback smoke
