...
R0 complete does **not** unfreeze. **DB-R1 (profiles) is now PASS AND CLOSED** — see `_documentation-audit/reports/DB-R1-PROFILES-RETIREMENT-CLOSE.md`. Freeze still **BLOCKED** pending DB-R2…R7 (modifiers prod apply, restaurant tables/QR, dine-in, kitchen, POS foundations, RLS extensions).
...
- `anon`: catalog `SELECT` only on `branches`, `menu_categories`, `menu_items`, `menu_item_variants`; residual DML on unmanaged `profiles` was cleared by DB-R1
- `authenticated`: catalog + orders family `SELECT`; `users` `SELECT`/`UPDATE`; `roles`/`user_roles` `SELECT`; residual DML on `profiles` cleared by DB-R1
...
**DB-R1** — **DONE** (see close report). Next owner gate: **DB-R2** modifiers production apply. Do not apply restaurant foundation slices without explicit approval.
