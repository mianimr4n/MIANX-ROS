...
## Happy path (operational)
1. Order created → `pending`
2. Staff confirms → kitchen ticket expected
3. Kitchen prepares → ready
4. Delivery assign (if delivery) → rider updates
5. Complete / cancel with status log

## Current honesty
- Pending orders must not imply kitchen tickets exist.
- Protected order `TP-260727-000001` remains pending without kitchen ticket until confirmation.
- Follow-on work on this branch will tighten OMS/KDS/delivery truth alignment.

## Data states
LOADING while fetching · LIVE on success · EMPTY when zero · ERROR/OFFLINE on failure (never fake LIVE zero).
