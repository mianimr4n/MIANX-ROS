...
| LIVE | Successful live API payload |
| DERIVED | Computed from LIVE inputs |
| EMPTY | Successful response with zero rows |
| STALE | Showing last success while refresh fails/pending |
| OFFLINE | Network/provider unreachable |
| ERROR | Request failed |
| FOUNDATION | Capability not implemented; honest placeholder |
| UNAVAILABLE | Cannot be shown safely |

## Labels
Owner / Founder = operating labels for super-admin. Do not create matching DB roles.
