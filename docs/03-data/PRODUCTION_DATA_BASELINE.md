...
## Protected baseline (do not mutate in readiness work)
| Entity | Verified baseline |
| --- | --- |
| Branches | royal-orchard = operating; northern-bypass = coming-soon |
| Orders | 1 pending order `TP-260727-000001` (Behari Roll) |
| Kitchen tickets | 0 until order confirmation |
| Reservations | EMPTY (0) on Royal Orchard smoke |
| Waitlist | EMPTY (0) on Royal Orchard smoke |
| Roles | Canonical eight staff roles; no owner/founder DB roles |

## Why this matters
Opening readiness must preserve the protected pending order and must not invent kitchen tickets or Northern Bypass operating status.
