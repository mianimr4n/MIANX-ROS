#!/usr/bin/env python3
"""Merge PR (Phase 7 closeout) via squash merge.

Usage:
  GITHUB_TOKEN=ghp_xxx HEAD_SHA=<sha> PR_NUMBER=<n> python3 scripts/merge_pr_phase_7.py
"""
...
PR_NUMBER = int(os.environ.get("PR_NUMBER", "234"))
...
if not TOKEN:
    ...
    sys.exit(1)
if not HEAD_SHA:
    print("ERROR: HEAD_SHA env var not set (pass the phase-7-closeout head SHA)", file=sys.stderr)
    sys.exit(1)
HEADERS = {
...
def main() -> int:
    ...
    payload = json.dumps({
        "commit_title": f"docs(v2.2.0): Phase 7 complete — ADR-023/024/025/026 + close report + roadmap update (#{PR_NUMBER})",
        "commit_message": "Phase 7 (POS System) formally closed: ADR-023 (Cashier Workflow), ADR-024 (Dine-in Bill Settlement), ADR-025 (POS Shifts/Z-Report/Cash Reconciliation), ADR-026 (Branch Sync/Offline-Safe) accepted. Closeout-only release — no new migrations; Production DB tip remains 20260821000000. All 26 ADRs (ADR-001 through ADR-026) Accepted v1.0.",
        "sha": HEAD_SHA,
        "merge_method": "squash",
    }).encode()
    req = urllib.request.Request(
